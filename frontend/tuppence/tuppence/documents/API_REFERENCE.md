...
All `/budgets` endpoints require `Authorization: Bearer <session_token>` and are
scoped to the authenticated user's household. Do not pass `household_id` —
it is inferred from the session.
...
**Response:** `BudgetResponse` (single budget with `id`)
...
// Load budgets from backend (household-scoped)
GET /budgets
...
All non-health endpoints require `Authorization: Bearer <session_token>`.
Get a token by signing in: `POST /auth/login` or `POST /auth/apple-signin`.
...
# List household's budgets
curl http://localhost:8000/budgets -H "Authorization: Bearer $TOKEN"

# Create a budget
curl -X POST http://localhost:8000/budgets \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"emoji":"🛒","label":"Groceries","monthly_amount":500}'
...
# Get amounts
curl http://localhost:8000/amounts -H "Authorization: Bearer $TOKEN"

# Export year
curl -H "Authorization: Bearer $TOKEN" \
  http://localhost:8000/export_year?year=2026 --output ledger.csv
```
